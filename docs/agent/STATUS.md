...
| current_phase | S1（B1 ACCEPTED；B2 ACCEPTED+CI PASS；B3 DRAFTED/AWAITING_DESIGN_CONFIRMATION） |
| current_work_order | S1-B3-WRITE-QUEUE-DURABILITY-BARRIER |
| work_order_state | DRAFTED / AWAITING_DESIGN_CONFIRMATION（实现冻结，等网页 ChatGPT 确认 A—H 设计） |
| current_state | PLAN_NEXT_BATCH |
| write_owner | Cursor |
| working_tree_state | S1-B2 关闭 + S1-B3 工作单起草，仅 metadata 文档变更；无业务源码改动 |
| base_code_sha | 1e140ca720a54dfa42cc37235484af7faae499bf（= S1-B2 accepted candidate） |
| candidate_code_sha | 无（S1-B3 未产出候选；实现待设计确认） |
| handoff_metadata_sha | 本 commit |
| github_push_state | S1-B2 候选 1e140ca 已推送且 GitHub Actions CI PASS（push 事件，conclusion=success） |
| latest_audit_verdict | S1-B2-CONFIRM-SEMANTICS-CLOSURE `ACCEPTED`（critical 0 / p1 0 / p2 0） |
| last_accepted_batch | S1-B2-CONFIRM-SEMANTICS-CLOSURE |
| last_accepted_code_sha | 1e140ca720a54dfa42cc37235484af7faae499bf |
| next_action | 网页 ChatGPT 确认 S1-B3 工作单设计（WORK_ORDER.md A—H）→ 才允许按单实现（RED Run L → 最小实现 → GREEN Run M/N → 候选 push → WAIT_AUDIT） |
...
| S1 | IN_PROGRESS | B1 CI/真实类失败测试 **ACCEPTED**（A08 经 GitHub 行为验证）；B2 写版本与回调 **ACCEPTED**；B3 队列/flush barrier/失败协议 **DRAFTED，设计待确认，未实现** | `evidence/s1-b1/`、`evidence/s1-b2/`；S1-B3 证据随实现批产出 |
| S2 | NOT_STARTED | 保存后应用（D04 握手，消费 FlushResult）、异步初始化/晋升/停止回执（D05）、双进程验证 | — |
| S3 | NOT_STARTED | 恢复只写一次、旧写入隔离、故障注入（D06；S1-B3 仅做 queue-fence 最小部分） | — |
...
2. readEpoch local-commit snapshot 保护（读取起点晚于本地提交的快照不得回滚）。
...
（测试锚点：`KvMemoryCacheTest` / `KvMemoryCacheLinearizabilityTest` / `RoomPreferenceDataStoreSnapshotOrderingTest`；S1-B3 实现批追加 `PreferenceWriteQueueTest` 系。）

## 遗留项

- P2-TEST-ROBUSTNESS：`SnapshotOrderingTest` 内 `Thread.sleep(80)` → 后续 test-infrastructure 清理批；**不在 S1-B3 顺手修**，不得为此扩生产设计。
...
S1-B3 为架构批：**先设计确认（本步），后实现**；不与禁止合并项混批。
